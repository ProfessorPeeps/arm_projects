...
;LEDs off, button up (switch open) 
STATE0
	LDR r7, [r1,#IO0PIN]
	MOV r8, #MASK14
	AND r9,r7,r8
...
;LEDs on, button down (switch closed)
STATE1
	LDR r7, [r1,#IO0PIN]
	LDR r8, =MASK2
	ORR r9,r7,r8
	TST	r9, r8
	BEQ STATE1				;If same means button is not pressed
	B	LEDon				;If Pin 14 pressed, branch LEDon

;LEDs on, button up (switch open again)
STATE2 
...
LEDon				;Turn on LEDs
	...
	LDR r6, =IO0_BASE		
	STR r5, [r6,#IO0SET]
	BX LR	;Check to see if still pressed	
